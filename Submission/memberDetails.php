...
<head>
  ...
  $query .= "FROM CORE.EVENT LEFT JOIN CORE.SIGNUP ON SIGNUP.EVENT_ID = EVENT.EVENT_ID LEFT JOIN CORE.EVENT_TYPE ON EVENT.EVENT_TYPE_ID = EVENT_TYPE.EVENT_TYPE_ID WHERE SIGNUP.MEMBER_ID='$member_id'";
  $result=mysqli_query($con,$query) or die("Query failed." . mysqli_error($con));
  ...
  // Sign in Query
  //Sign Up Query
  $query="SELECT event.EVENT_ID AS eid, event.NAME AS ename, event_type.name AS etype, LOCATION, DATE_FORMAT(start_date,'%c/%e/%Y') AS sdate, DATE_FORMAT(start_time,'%l:%i %p') AS stime, DATE_FORMAT(end_time,'%l:%i %p') AS etime  FROM core.signin LEFT JOIN core.event ON signin.event_id = event.event_id LEFT JOIN core.event_type ON event.event_type_id = event_type.event_type_id WHERE signin.member_id='$member_id'";
  $result=mysqli_query($con,$query) or die("Query failed." . mysqli_error($con));
